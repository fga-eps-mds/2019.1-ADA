            ...
            try:
                ...
                try:
                    ...
                    if not pull_request["pull_request"]:
                        ...
                        pr_len = len(pull_request["pull_request"])
                        if(pr_len > 5):
                            pull_request["pull_request"] = (
                                pull_request["pull_request"][:5])
                        pr_size = len(pull_request["pull_request"])
                        pull_request_text = "🛠 Pull requests:\n"
                        pull_request_text += "Existem {pull_request_size} "\
                                             "pull's requests abertos no"\
                                             " seu repositório.\n".format(
                                              pull_request_size=pr_len)
                        pull_request_text += "Estes são os últimos "\
                                             "{pull_request_size} pull "\
                                             "request's:\n".format(
                                              pull_request_size=pr_size)
